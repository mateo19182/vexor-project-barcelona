  ...
  return (
    <div className="p-6 h-full overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Execution Timeline</h2>
        <span className="text-xs text-text-tertiary">{totalTime.toFixed(2)}s total</span>
      </div>
              ...
              <div className="space-y-1.5">
                {mods.map((m) => (
                  <div key={m.name} className="flex items-center gap-3">
                    <span className="text-xs text-text-secondary w-32 truncate shrink-0 font-mono">
                      {m.name}
                    </span>
                    <div className="flex-1 h-7 bg-bg-elevated/50 rounded relative overflow-hidden">
                      <div
